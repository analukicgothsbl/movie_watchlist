...
def index():
    ...
    movie_data = current_app.db.movies.find({"_id": {"$in": current_user.movies}})
    movies = [Movie(**mov) for mov in movie_data]
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user_data:
            flash("User credentials not correct", category="danger")
            return redirect(url_for(".login"))
...
def add_movie():
    ...
    if form.validate_on_submit():
        ...
        # turn object to dictionary using asdict func from dataclass
...
@pages.get("/movie/<string:_id>/rating")
@login_required
